 ...
 * Field names are all `wpmphub_settings[key]`, so the Settings API hands the whole
 * tab to one sanitise callback.
 *
 * A field declaring `confirm` renders a `data-wpmphub-confirm` attribute carrying
...
final class SettingsRenderer {
	...
	public function render_row( string $key, array $field ): void {
		$id    = 'wpmphub-field-' . sanitize_key( $key );
		$label = (string) ( $field['label'] ?? $key );
		$help  = (string) ( $field['help'] ?? '' );
		$type  = (string) ( $field['type'] ?? 'bool' );

		printf(
			'<div class="wpmphub-settings__row wpmphub-settings__row--%1$s" data-wpmphub-settings-field="%2$s" data-wpmphub-settings-terms="%3$s">',
		...
		printf(
			'<div class="wpmphub-settings__row-label"><label for="%1$s">%2$s</label></div>',
			esc_attr( $id ),
			esc_html( $label )
		);

		echo '<div class="wpmphub-settings__row-field">';

		$this->render_control( $key, $field, $id );

		if ( '' !== $help ) {
			printf( '<p class="wpmphub-settings__help">%s</p>', esc_html( $help ) );
		}
	...
	private function checkbox( string $key, array $field, string $id, bool $checked ): void {
		...
		printf(
			'<label class="wpmphub-switch"><input type="checkbox" id="%1$s" name="%2$s" value="1" %3$s%4$s /><span class="wpmphub-switch__track" aria-hidden="true"></span><span class="wpmphub-switch__label" data-wpmphub-switch-states="%5$s">%6$s</span></label>',
			esc_attr( $id ),
	...
	private function number( string $key, array $field, string $id, int $value ): void {
		printf(
			'<input type="number" class="wpmphub-settings__number" id="%1$s" name="%2$s" value="%3$d" min="%4$d" max="%5$d" step="1" />',
			esc_attr( $id ),
			esc_attr( self::name( $key ) ),
			(int) $value,
	...
	private function dates( string $key, string $id, array $dates ): void {
		printf(
			'<textarea id="%1$s" name="%2$s" rows="5" class="wpmphub-settings__textarea code" placeholder="2026-12-25">%3$s</textarea>',
			esc_attr( $id ),
			esc_attr( self::name( $key ) ),
	...
	private static function confirm_attribute( array $field, $trigger ): string {
		...
		return sprintf(
			' data-wpmphub-confirm="%1$s" data-wpmphub-confirm-value="%2$s"',
