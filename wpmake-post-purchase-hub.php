...
define( 'WPMPHUB_VERSION', '1.0.0' );
define( 'WPMPHUB_PLUGIN_FILE', __FILE__ );
define( 'WPMPHUB_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );
define( 'WPMPHUB_PLUGIN_URL', plugin_dir_url( __FILE__ ) );
define( 'WPMPHUB_MINIMUM_PHP', '8.1' );
define( 'WPMPHUB_MINIMUM_WP', '6.5' );
define( 'WPMPHUB_MINIMUM_WC', '10.9' );
 ...
 */
define( 'WPMPHUB_EDITION', 'free' );

if ( is_readable( WPMPHUB_PLUGIN_DIR . 'vendor/autoload.php' ) ) {
	require_once WPMPHUB_PLUGIN_DIR . 'vendor/autoload.php';
}
...
add_action( 'plugins_loaded', 'wpmphub_bootstrap' );
 ...
 * @return void
 */
function wpmphub_bootstrap(): void {
	...
	if ( array() !== $failures ) {
		add_action(
			...
			static function () use ( $failures ): void {
				wpmphub_requirements_notice( $failures );
			}
		);
	...
	add_action( 'before_woocommerce_init', 'wpmphub_declare_hpos_compatibility' );

	// Before register(), which is what fires `wpmphub_loaded`: an edition that
	// attaches after that hook has already fired would never run.
	wpmphub_load_edition();
 ...
 * @return void
 */
function wpmphub_load_edition(): void {
	foreach ( array( 'pro/bootstrap.php', 'free/bootstrap.php' ) as $wpmphub_bootstrap ) {
		$wpmphub_path = WPMPHUB_PLUGIN_DIR . $wpmphub_bootstrap;

		if ( is_readable( $wpmphub_path ) ) {
			require_once $wpmphub_path;
		}
	}
}
 ...
 * @return bool
 */
function wpmphub_is_pro(): bool {
	return defined( 'WPMPHUB_EDITION' ) && 'pro' === WPMPHUB_EDITION;
}
 ...
 */
function wpmphub_requirement_failures( string $php_version, string $wp_version, ?string $wc_version, bool $autoloaded = true ): array {
	$failures = array();

	if ( version_compare( $php_version, WPMPHUB_MINIMUM_PHP, '<' ) ) {
		$failures[] = sprintf(
	...
	if ( version_compare( $wp_version, WPMPHUB_MINIMUM_WP, '<' ) ) {
		$failures[] = sprintf(
	...
	} elseif ( version_compare( $wc_version, WPMPHUB_MINIMUM_WC, '<' ) ) {
		$failures[] = sprintf(
 ...
 * @since 0.1.0
 *
 * @param string[] $failures Unmet requirements, as returned by wpmphub_requirement_failures().
 * @return void
 */
function wpmphub_requirements_notice( array $failures ): void {
 ...
 * @return void
 */
function wpmphub_declare_hpos_compatibility(): void {
	if ( class_exists( Automattic\WooCommerce\Utilities\FeaturesUtil::class ) ) {
		Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility( 'custom_order_tables', WPMPHUB_PLUGIN_FILE, true );
	}
}
