...
- **WooCommerce must come before `"."` in `.wp-env.json`'s `plugins` array.** wp-env activates them in the order listed, and this plugin's `Requires Plugins: woocommerce` header makes WordPress refuse to activate it while WooCommerce is not active yet — so listing ours first fails the whole `wp-env start`, before any test runs, with "Failed to activate plugin … requires 1 plugin". JSON cannot carry a comment saying so, which is why it is written down here.
