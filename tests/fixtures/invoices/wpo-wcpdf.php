...
if ( ! function_exists( 'wcpdf_get_document' ) ) {
	 ...
	 * @return WPMPHUB_Fixture_Wpo_Wcpdf_Document
	 * @throws RuntimeException When the fixture is set to throw.
	 */
	function wcpdf_get_document( string $document_type, $order, bool $init = false ): WPMPHUB_Fixture_Wpo_Wcpdf_Document {
		unset( $document_type, $order, $init );

		if ( WPMPHUB_Fixture_Wpo_Wcpdf::$throws ) {
			throw new RuntimeException( 'Fixture failure inside the invoice plugin.' );
		}

		return new WPMPHUB_Fixture_Wpo_Wcpdf_Document();
	}
}

// phpcs:enable WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedFunctionFound

if ( ! function_exists( 'wpmphub_fixture_register_wcpdf_shortcode' ) ) {
	/**
	 * Registers the fixture's document-link shortcode.
	 *
	 * @return void
	 */
	function wpmphub_fixture_register_wcpdf_shortcode(): void {
		add_shortcode(
			'wcpdf_document_link',
			static function ( $atts = array() ): string {
				WPMPHUB_Fixture_Wpo_Wcpdf::$last_atts = is_array( $atts ) ? $atts : array();

				return WPMPHUB_Fixture_Wpo_Wcpdf::$link_output;
			}
		);
	}
}
